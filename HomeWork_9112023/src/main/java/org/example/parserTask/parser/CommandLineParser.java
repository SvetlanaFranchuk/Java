package org.example.parserTask.parser;
...
public class CommandLineParser {
    ...
    public static boolean isDateTypeCorrect(String dateType) throws CommandLineParserException {
        if (!(dateType.charAt(1) == 's' || dateType.charAt(1) == 'i'))
            throw new CommandLineParserException("The key for dateType is not correct");
        return true;
    }

    public static boolean isFileNameNotEmpty(String fileName) throws CommandLineParserException {
        if (fileName.charAt(0) == '-' || fileName.charAt(0) == ' ')
                throw new CommandLineParserException("File name is empty");
        return true;
    }

    public static boolean hasListNextFileDate(List<String> list) throws CommandLineParserException {
